...
import com.moringa.david.bookstore.model.Author;
import com.moringa.david.bookstore.model.AuthorBuilder;
...
import com.moringa.david.bookstore.util.CircleTransform;
...
public class AuthorViewActivity extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (cache.getCachedAuthorById(author.getId()) == null) {
            mGoodreadRequest.getAuthor(author.getId(), new com.moringa.david.bookstore.network.SuccessFailCallback() {
                @Override
    ...
    private void updateDetails(Author author) {
        ...
        for (int i = 0; i < Math.min(6, bookIds.size()); i++) {
            ...
            if (cache.getCachedBookById(bookIds.get(i)) == null) {
                mGoodreadRequest.getBook(bookIds.get(i), new com.moringa.david.bookstore.network.SuccessFailCallback() {
                    @Override
                    public void success(String response) {
