...
class SearchPresenter {
    ...
    /**
     * Gets a list of Goodreads ids from the XML response of {@link GoodreadRequest#searchBook(String, com.moringa.david.bookstore.network.SuccessFailCallback)} API
     *
    ...
    public void searchQuery(String query, final GoodreadRequest goodreadRequest, final InternalStorage cache) {
        ...
        goodreadRequest.searchBook(query, new com.moringa.david.bookstore.network.SuccessFailCallback() {
            @Override
    ...
    private void getEachBook(List<Integer> bookIds, final GoodreadRequest goodreadRequest, final InternalStorage cache){
        ...
        for (int i = 0; i < bookIds.size(); i++) {
            ...
            if (cache.getCachedBookById(bookIds.get(i)) == null) {
                goodreadRequest.getBook(bookIds.get(i), new com.moringa.david.bookstore.network.SuccessFailCallback() {
                    @Override
                    public void success(String response) {
     ...
     *
     * @param xmlString XML string from {@link GoodreadRequest#searchBook(String, com.moringa.david.bookstore.network.SuccessFailCallback)} API
